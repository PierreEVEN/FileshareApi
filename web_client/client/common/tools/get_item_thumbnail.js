    ...
    switch (mime[0]) {
        case 'video':
            return `<div class="item-small">
                            <img class="item-background" src="${PAGE_CONTEXT.repos_path()}/thumbnail/${item.id}" alt="fichier: '${item.name}" onError="this.onError = null; this.src='/public/images/icons/mime-icons/video.png'"/>
                            <img class="item-overlay" src="/public/images/icons/icons8-play-64.png" alt="play button">
                        </div>`
        case 'image':
            return `<img class="item-small" src="${PAGE_CONTEXT.repos_path()}/thumbnail/${item.id}" alt="fichier: ${item.name}" onError="this.onError = null; this.src='/public/images/icons/mime-icons/image.png'"/>`
        case 'application':
            switch (mime[1]) {
                case 'x-pdf':
                case 'pdf':
...
function from_local_path(item) {
    switch (item.mimetype.split('/')[0]) {
        case 'image':
            return `<img class="item-small" src="${URL.createObjectURL(item)}" alt="image: ${item.name}" onError="this.onError = null; this.src='/public/images/icons/mime-icons/image.png'"/>`
        case 'video':
