...
impl AsyncDirectoryZip {
    ...
    pub async fn push_item(&mut self, item: Item) -> Result<(), Error> {
        let mut items_to_push = vec![item];
        while let Some(item) = items_to_push.pop() {
            if item.directory.is_some() {
                let children = Item::from_parent(&self.ctx.database, item.id(), Trash::No).await?;
    ...
    pub async fn finalize<S: Unpin + AsyncWrite>(&mut self, mut sink: S) -> Result<(), Error> {
        ...
        for item in self.items.values() {
            ...
            let crc32 = if let Some(file) = &item.file {
                let found_object = Object::from_id(&self.ctx.database, &file.object).await?;
                let crc = Self::compute_file_crc(File::open(found_object.data_path(&self.ctx.database))?)?;
            ...
            if let Some(object) = &object {
                let mut file = File::open(object.data_path(&self.ctx.database))?;
